...
/*char[] vowels = { 'A', 'E', 'I', 'O', 'U', 'Y', 'a', 'e', 'i', 'o', 'y'};

var vowelName = debtors.Where(d => vowels.Contains(d.FullName[0])).ToList();
vowelName.ForEach(Console.WriteLine);*/

//  2. Вывести тех должников, имеющих почту в домене rhyta.com или dayrep.com

/*var emailChoise = debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).ToList();
emailChoise.ForEach(Console.WriteLine);*/

//  3. Вывести тех должников в возрасте от 26 до 36 лет.

/*var ageChoise = debtors.Where(d => DateTime.Now.Year - d.BirthDay.Year >= 26 && DateTime.Now.Year - d.BirthDay.Year <= 36).ToList();
ageChoise.ForEach(Console.WriteLine);*/

//  4. Вывести тех должников, сумма долга которых не превышает 5000.

/*var debtChoise = debtors.Where(d => d.Debt <= 5000).ToList();
debtChoise.ForEach(Console.WriteLine);*/

//  5. Вывести тех должников, полное имя которых длиннее 18 символов, и номер телефона которых содержит 2 или больше семерок.

/*var name18 = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(n => n == '7') >=2).ToList();
name18.ForEach(Console.WriteLine);*/
...
/*var winterBorn = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2).OrderBy(i => i.Address[^5..]).ToList();
winterBorn.ForEach(Console.WriteLine);*/
...
/*Console.WriteLine(debtors.Average(d => d.Debt));
var aboveAverage = debtors.Where(d => d.Debt > debtors.Average(a => a.Debt));
aboveAverage.OrderBy(o => o.FullName[o.FullName.LastIndexOf(' ')..]).ToList().ForEach(Console.WriteLine);
Console.WriteLine();
aboveAverage.OrderByDescending(o => o.Debt).ToList().ForEach(Console.WriteLine);*/

//  9. Вывести только фамилии, возраст и суммы долга всех должников, в номере телефона которых отсутствуют восьмерки.

/*var without8 = debtors.Where(d => d.Phone.Contains('8') is not true).ToList();
